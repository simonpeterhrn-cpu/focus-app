...
import {
  Check, Flame, Plus, Trash2, ListChecks,
...
export default function App() {
  ...
  const [timer, setTimer] = useState({
    mode: "work", running: false, secondsLeft: 25 * 60,
  ...
  useEffect(() => {
    ...
    const id = setInterval(() =>
      setTimer((t) => t.secondsLeft > 0 ? { ...t, secondsLeft: t.secondsLeft - 1 } : t), 1000);
  ...
  useEffect(() => {
    ...
    playChime();
    if (timer.mode === "work") {
      addDeepWork(timer.workMin); bumpPomodoro();
      notify("Break time!", `${timer.breakMin}-minute break starting now.`);
      setTimer((t) => ({ ...t, mode: "break", secondsLeft: t.breakMin * 60, running: true }));
    } else {
      notify("Break over", "Ready for the next focus session?");
      setTimer((t) => ({ ...t, mode: "work", secondsLeft: t.workMin * 60, running: false }));
    }
  }, [timer.secondsLeft, timer.running]); // eslint-disable-line
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Geist', system-ui, sans-serif" }}>
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "24px 16px 88px" }}>
        ...
        {page === "Focus" && <FocusPage data={data} timer={timer} setTimer={setTimer} />}
        ...
        {page === "Growth" && <GrowthPage data={data} setData={setData} habits={habits} />}
      </div>

      <nav style={{
        position: "fixed", bottom: 0, left: 0, right: 0,
        background: C.panel, borderTop: `1px solid ${C.border}`,
        display: "flex", padding: "8px 8px 14px",
      }}>
          ...
          {NAV.map(({ id, Icon }) => {
            ...
            return (
              <button key={id} onClick={() => setPage(id)} style={{
                ...
                color: on ? C.sage : C.dim, padding: "6px 0",
              }}>
    ...
    <>
      ...
      <div style={{ ...card, marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 7 }}>
          {[1, 2, 3, 4, 5].map((n) => (
            <button key={n} onClick={() => setData((d) => ({ ...d, energy: n }))} style={{
              width: 20, height: 20, borderRadius: "50%", border: "none", cursor: "pointer", padding: 0,
              background: n <= data.energy ? C.sage : C.elevated, transition: "background 0.15s",
            }} />
          ))}
        </div>
      </div>
      ...
      <div style={{ ...card, marginBottom: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12 }}>
          ...
          <span style={{ fontFamily: "'Geist Mono', monospace", fontSize: 13, fontWeight: 700, color: allDone ? C.sage : C.dim }}>
            {done}/{total}
          </span>
        </div>
        <div style={{ height: 6, background: C.elevated, borderRadius: 99, marginBottom: 14, overflow: "hidden" }}>
          <div style={{
            ...
            background: allDone ? C.sage : C.sand,
            boxShadow: pct > 0 ? `0 0 8px ${allDone ? "rgba(126,184,160,0.45)" : "rgba(196,168,130,0.35)"}` : "none",
        ...
        <div style={{ display: "grid", gap: 7 }}>
          {data.tasks.map((t) => (
            <div key={t.id} style={tRow}>
              ...
              <span style={{ flex: 1, fontSize: 15, opacity: t.done ? 0.3 : 1, textDecoration: t.done ? "line-through" : "none", transition: "opacity 0.2s" }}>
                {t.label}
              </span>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
          <input value={newTask} onChange={(e) => setNewTask(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && add()} placeholder="Add a task..." style={txIn} />
        ...
        <div style={card}>
          ...
          <div style={{ marginTop: 14, display: "grid", gap: 10 }}>
            <div>
              ...
              <textarea value={todayEntry.win} onChange={(e) => patchJournal({ win: e.target.value })}
                placeholder="What went well today?" rows={2} style={txArea} />
            </div>
            <div>
              <div style={{ fontSize: 12, color: C.dim, marginBottom: 6 }}>Gratitude</div>
              <textarea value={todayEntry.gratitude} onChange={(e) => patchJournal({ gratitude: e.target.value })}
                placeholder="Something you're grateful for..." rows={2} style={txArea} />
            </div>
          </div>
        </div>
      )}
...
// ---------------------------------------------------------------------------
function FocusPage({ data, timer, setTimer }) {
  const total = (timer.mode === "work" ? timer.workMin : timer.breakMin) * 60;
  const frac = total ? 1 - timer.secondsLeft / total : 0;
  ...
  const toggle = () => setTimer((t) => ({ ...t, running: !t.running }));
  const reset = () => setTimer((t) => ({ ...t, running: false, secondsLeft: (t.mode === "work" ? t.workMin : t.breakMin) * 60 }));
  const skip = () => setTimer((t) => t.mode === "work"
    ? { ...t, mode: "break", secondsLeft: t.breakMin * 60, running: false }
    : { ...t, mode: "work", secondsLeft: t.workMin * 60, running: false });
  const preset = (w, b) => setTimer((t) => ({ ...t, workMin: w, breakMin: b, mode: "work", secondsLeft: w * 60, running: false }));

  return (
    <>
      <h1 style={{ margin: "0 0 4px", fontSize: 26, fontWeight: 800, letterSpacing: "-0.02em" }}>Focus</h1>
      <p style={{ margin: "0 0 18px", color: C.dim, fontSize: 14 }}>
        Work in focused blocks. Each session logs your deep-work hours.
      </p>

      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {MODES.map((m) => (
          <button key={m} onClick={() => setTimer((t) => ({ ...t, focusMode: m }))} style={{
            flex: 1, padding: "9px 0", borderRadius: 10, fontSize: 12, fontWeight: 600, cursor: "pointer",
            background: timer.focusMode === m ? C.sand : C.elevated,
            color: timer.focusMode === m ? C.bg : C.dim,
            border: `1px solid ${timer.focusMode === m ? C.sand : C.border}`,
      ...
      <div style={{ ...card, display: "flex", flexDirection: "column", alignItems: "center", padding: "28px 20px" }}>
        <div style={{
          ...
          color: timer.mode === "work" ? C.sand : C.sage,
          padding: "4px 14px", borderRadius: 99,
          background: timer.mode === "work" ? C.sandDim : C.sageDim,
          border: `1px solid ${timer.mode === "work" ? "rgba(196,168,130,0.3)" : "rgba(126,184,160,0.3)"}`,
          transition: "all 0.3s",
        }}>
        ...
        <div style={{ position: "relative", width: size, height: size, margin: "16px 0" }}>
          <svg width={size} height={size} style={{
            ...
            filter: timer.running ? `drop-shadow(0 0 12px ${ringColor}99)` : "none",
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <button onClick={toggle} style={{
            ...
            background: ringColor, color: C.bg, border: "none", borderRadius: 12,
        ...
        <div style={{ marginTop: 16, fontFamily: "'Geist Mono', monospace", fontSize: 12, color: C.dim, background: C.elevated, borderRadius: 99, padding: "5px 16px", border: `1px solid ${C.border}` }}>
          <strong style={{ color: C.text }}>{data.pomodorosToday}</strong>{" "}
          session{data.pomodorosToday !== 1 ? "s" : ""} today
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        {[[25, 5], [50, 10]].map(([w, b]) => {
          ...
          return (
            <button key={w} onClick={() => preset(w, b)} style={{
              flex: 1, background: on ? C.elevated : "transparent", color: on ? C.text : C.dim,
              border: `1px solid ${on ? C.sand : C.border}`, borderRadius: 12,
              padding: "12px", fontSize: 14, fontWeight: 600, cursor: "pointer", transition: "all 0.15s",
            }}>{w} / {b} min</button>
          );
        })}
      </div>
...
function HabitsPage({ habits, setHabits }) {
  ...
  const addHabit = () => {
    ...
    setHabits((h) => [...h, { id: Date.now(), name: newName.trim(), area: newArea, log: {} }]);
    setNewName("");
  };
              ...
              {aHabits.map((h) => {
                ...
                return (
                  <div key={h.id} style={{
                    ...
                    border: `1px solid ${doneToday ? aColor + "55" : C.border}`,
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 3, marginBottom: 10 }}>
                      {days.map((d) => (
                        <div key={d} style={{
                          ...
                          background: h.log[d] ? aColor : C.elevated,
                          outline: d === today ? `1.5px solid ${aColor}` : "none",
                          outlineOffset: 1,
                        }} />
                      ))}
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      <button onClick={() => toggleHabit(h.id)} style={{
                        ...
                        background: doneToday ? aColor : C.elevated,
                        border: `1.5px solid ${doneToday ? aColor : C.border}`,
      ...
      <div style={card}>
        ...
        <div style={{ display: "flex", gap: 8, margin: "12px 0 10px" }}>
          <input value={newName} onChange={(e) => setNewName(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && addHabit()} placeholder="Habit name..." style={txIn} />
        ...
        <div style={{ display: "flex", gap: 7 }}>
          {AREAS.map((a) => (
            <button key={a} onClick={() => setNewArea(a)} style={{
              ...
              background: newArea === a ? AREA_COLORS[a] : C.elevated,
              color: newArea === a ? C.bg : C.dim,
              border: `1px solid ${newArea === a ? AREA_COLORS[a] : C.border}`,
              transition: "all 0.15s",
            }}>{a}</button>
          ))}
        </div>
  ...
  return (
    <>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 20 }}>
        ...
        <button onClick={() => setWeekMode(!weekMode)} style={{
          fontSize: 12, color: weekMode ? C.sage : C.dim, background: C.elevated,
          border: `1px solid ${weekMode ? C.sage : C.border}`, borderRadius: 8,
        ...
        <div style={{ display: "grid", gap: 10 }}>
          <div style={card}>
            ...
            <div style={{ display: "flex", gap: 7, marginTop: 12 }}>
              {[1, 2, 3, 4, 5].map((n) => (
                <button key={n} onClick={() => patchJournal({ mood: n })} style={{
                  flex: 1, padding: "9px 0", borderRadius: 9, fontSize: 11, fontWeight: 600, cursor: "pointer",
                  background: todayEntry.mood === n ? C.sage : C.elevated,
                  color: todayEntry.mood === n ? C.bg : C.dim,
                  border: `1px solid ${todayEntry.mood === n ? C.sage : C.border}`,
                  transition: "all 0.15s",
                }}>{MOODS[n]}</button>
              ))}
            </div>
          ...
          <div style={card}>
            ...
            <textarea value={todayEntry.win} onChange={(e) => patchJournal({ win: e.target.value })}
              placeholder="What went well today?" rows={3} style={{ ...txArea, marginTop: 10 }} />
          </div>

          <div style={card}>
            <div style={secLabel}>Gratitude</div>
            <textarea value={todayEntry.gratitude} onChange={(e) => patchJournal({ gratitude: e.target.value })}
              placeholder="Something you're grateful for..." rows={3} style={{ ...txArea, marginTop: 10 }} />
          </div>

          <div style={card}>
            <div style={secLabel}>What's holding you back?</div>
            <textarea value={todayEntry.friction} onChange={(e) => patchJournal({ friction: e.target.value })}
              placeholder="Friction, blockers, or fears..." rows={3} style={{ ...txArea, marginTop: 10 }} />
          </div>
        </div>
      ) : (
              ...
              <div key={d} style={{ ...card, padding: "14px 16px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    {entry.mood > 0 && (
                      <span style={{ fontSize: 11, color: C.sage, fontWeight: 600 }}>{MOODS[entry.mood]}</span>
                    )}
                    <div style={{ width: 7, height: 7, borderRadius: "50%", background: entry.win ? C.sage : C.border }} />
                  </div>
                </div>
                {entry.win && (
                  <div style={{ fontSize: 13, color: C.dim, marginTop: 8, lineHeight: 1.55 }}>{entry.win}</div>
                )}
              </div>
            );
          })}
...
// ---------------------------------------------------------------------------
function GrowthPage({ data, setData, habits }) {
  ...
  const days = [...data.history].sort((a, b) => a.day.localeCompare(b.day)).slice(-7);
  const chartData = days.map((h) => ({ name: shortDay(h.day), hours: h.hours, completed: h.completed }));
  ...
  const habitAreas = AREAS.map((area) => ({
    area,
    total: habits.filter((h) => h.area === area).length,
  ...
  return (
    <>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 4, right: 4, left: -22, bottom: 0 }}>
                ...
                <Tooltip cursor={{ fill: "rgba(255,255,255,0.04)" }}
                  contentStyle={{ background: C.elevated, border: `1px solid ${C.border}`, borderRadius: 8, color: C.text, fontSize: 12 }} />
            ...
            {habitAreas.map(({ area, total, done }) => {
              ...
              return (
                <div key={area} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <span style={{ fontFamily: "'Geist Mono', monospace", fontSize: 11, color: C.dim, width: 32, textAlign: "right" }}>
                    {done}/{total}
                  </span>
                </div>
              );
      ...
      <div style={card}>
        ...
        {data.goals.length === 0 && (
          <div style={{ color: C.dim, fontSize: 14, padding: "6px 0 10px" }}>No goals yet. Add the first one below.</div>
        )}
        <div style={{ display: "grid", gap: 7, marginBottom: data.goals.length ? 12 : 0 }}>
          {data.goals.map((g) => (
            <div key={g.id} style={tRow}>
              ...
              <span style={{ flex: 1, fontSize: 15, opacity: g.done ? 0.3 : 1, textDecoration: g.done ? "line-through" : "none", transition: "opacity 0.2s" }}>
                {g.text}
              </span>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input value={newGoal} onChange={(e) => setNewGoal(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && addGoal()} placeholder="Add a goal..." style={txIn} />
